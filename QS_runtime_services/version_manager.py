...
# 启动时和“版本信息”按钮输出的版本说明。
VERSION_INFO_TITLE = "B2026.3版主要更新内容如下："
VERSION_INFO_ITEMS = [
    "---界面设计",
    "【端口处理】合并了多个端口处理的功能",
    "【参数信息】将'频点列表/文件信息/频域切片'合并到右上「参数信息 ▾」下拉，节省横向空间",
    "【信息反馈】左侧新增'评价 | 需求反馈'入口，支持文本+截图附件直接提交",
    "---功能增加",
    "【版本管理】新增版本检查功能，有新版本发布自动提醒更新",
    "【文件列表】新增完整路径/文件名显示模式切换，长路径文件可简化显示",
    "【数据复用】频域分析、时域分析、纹波拟合和端口合并统一复用主窗口缓存数据",
    "【拓扑识别】自动识别1驱1/1驱多端口关系判断，可叠加S/Z/Y参数矩阵热力图（未完成测试）",
    "【主绘图UI】增加时域模块，支持UI宽度、频域窗等参数选择（未完成测试）",
    "---交互体验",
    "【交互式Legend】可修改/移动/关闭；点击图例条目可高亮对应曲线，与点击曲线本体行为一致",
    "【频域分析】修复文件名显示模式下指定线号、最差bit和XTSum等分析的数据访问问题",
    "【差分转换】重排对话框布局，端口示意图左右并排显示，修复端口数解析",
    "---Bug修复",
    "重构了绘图代码，修复部分绘图单位缺失的问题",
    "统一缓存管理，新增S/Y/Z参数统一懒加载缓存、文件指纹校验和缓存调试开关",
]
